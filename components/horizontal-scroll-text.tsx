...
export function HorizontalScrollText({ text, direction = "left", className = "" }: HorizontalScrollTextProps) {
  ...
  const x = useTransform(scrollYProgress, [0, 1], direction === "left" ? ["-50%", "0%"] : ["0%", "-50%"])

  return (
    <div ref={containerRef} className={`relative overflow-hidden py-12 ${className}`}>
      <motion.div className="whitespace-nowrap text-[4rem] md:text-[8rem] font-bold text-black/5 flex" style={{ x }}>
